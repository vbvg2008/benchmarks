...
def get_estimator():
    ...
    schedule1 = RepeatScheduler(repeat_list=[
        get_tensorflow_dataset(x=np.expand_dims(x_train, -1), y=y_train),
        get_tensorflow_dataset(x=np.expand_dims(x_train, -1), y=y_train)
    ])
    schedule2 = EpochScheduler(
        epoch_dict={
            0: get_tensorflow_dataset(x=np.expand_dims(x_eval, -1), y=y_eval),
