...
def run_tuning(tasks, task_weights, log_file, num_trials=20000):
    print("Begin tuning...")
    measure_ctx = auto_scheduler.LocalRPCMeasureContext(repeat=1, min_repeat_ms=300, timeout=10)
    if os.path.exists(log_file):
        tuner = auto_scheduler.TaskScheduler(tasks, task_weights, load_log_file=log_file)
        num_trials = num_trials - file_len(log_file)
    else:
...
def run(d, hw):
    ...
    if d == "cpu":
        ...
        device = tvm.cuda(0)
    log_file = "%s-%s-B%d-%s.json" % (network, hw, batch_size, target.kind.name)
    print("Tuning...")
