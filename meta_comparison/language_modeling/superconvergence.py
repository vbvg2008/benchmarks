...
def linear_increase(step, min_lr=0.0, max_lr=100.0, num_steps=1000):
    lr = step / num_steps * (max_lr - min_lr) + min_lr
    return lr


def get_estimator(data_dir, seq_length=20, batch_size=128, vocab_size=10000, epochs=500):
    ...
    model = fe.build(model_fn=lambda: build_model(vocab_size, embedding_dim=300, rnn_units=600, batch_size=batch_size),
                     optimizer_fn=lambda: tf.optimizers.SGD(3.0, momentum=0.9))  #1.0, 0.1, 0.01
