...
def get_estimator(epochs=310, batch_size=128, save_dir=tempfile.mkdtemp(), max_train_steps_per_epoch=None):
    ...
    model = fe.build(model_fn=densenet, optimizer_fn=lambda: tf.optimizers.SGD(0.05))
    ...
    traces = [
        ...
        LRScheduler(model=model, lr_fn=lambda epoch: cosine_decay(epoch, cycle_length=10, init_lr=0.05, min_lr=0.001))
    ]
