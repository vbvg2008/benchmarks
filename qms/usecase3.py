...
def get_estimator(batch_size=32):
    ...
    traces = [
        ...
        QMSTest(
            test_descriptions=[
                "Accuracy should be above 0.9",
                "MCC score should be above 0.95",
                "Accuracy should be above 0.95 and MCC should be below 0.9"
            ],
            test_criterias=[
                lambda accuracy: accuracy > 0.9,
                lambda mcc: mcc > 0.95,
                lambda accuracy, mcc: accuracy > 0.95 and mcc < 0.9
            ],
            test_title="Mnist Classification QMS Test",
            output_path="XD_QMS2.json")
    ]
