...
def super_schedule(step, lr_max=0.03, lr_min=0.0075, steps_per_epoch=1500):
    if step < steps_per_epoch * 13:
        lr = (lr_max - lr_min) / steps_per_epoch / 13 * step + lr_min
    elif step < steps_per_epoch * 26:
        lr = lr_max - (step - steps_per_epoch * 13) / (steps_per_epoch * 26 - steps_per_epoch * 13) * (lr_max - lr_min)
    else:
        lr = lr_min - (step - steps_per_epoch * 26) / (steps_per_epoch * 30 - steps_per_epoch * 26) * (lr_min - 0.0)
    return lr


def get_estimator(data_dir, max_train_steps_per_epoch=1500, batch_size=32, epochs=30, image_size=256, num_classes=90):
    # pipeline
    ...
    model = fe.build(model_fn=lambda: RetinaNet(input_shape=(image_size, image_size, 3), num_classes=num_classes),
                     optimizer_fn=lambda: tf.optimizers.SGD(0.0075, momentum=0.9))
