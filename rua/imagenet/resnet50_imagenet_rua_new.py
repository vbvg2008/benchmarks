...
def get_estimator(level, data_dir, max_N=3, batch_per_gpu=256, epochs=180, save_dir=tempfile.mkdtemp()):
    ...
    aug_ops = [Sometimes(op, prob=min(level / 30 * max_N / len(aug_options), 1.0)) for op in aug_options]
    pipeline = Pipeline(
        train_data=train_ds,
        eval_data=eval_ds,
