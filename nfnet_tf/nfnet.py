...
class NFNet(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.stem = tf.keras.Sequential([
            self.which_conv(16, kernel_size=3, strides=2, padding='same', name='stem_conv0'),
            self.activation,
            self.which_conv(32, kernel_size=3, strides=1, padding='same', name='stem_conv1'),
            self.activation,
            self.which_conv(64, kernel_size=3, strides=1, padding='same', name='stem_conv2'),
            self.activation,
            self.which_conv(ch, kernel_size=3, strides=2, padding='same', name='stem_conv3'),
        ])
        self.blocks = []
        expected_std = 1.0
        ...
        block_args = zip(self.width_pattern,
                         self.depth_pattern,
                         self.bneck_pattern,
                         self.group_pattern,
                         self.big_pattern,
...
class NFBlock(tf.keras.Model):
    def __init__(self,
        ...
        self.conv1 = which_conv(filters=self.width,
        ...
        if self.use_two_convs:
            self.conv1b = which_conv(filters=self.width,
                                     kernel_size=kernel_shape,
                                     strides=1,
                                     padding='same',
                                     groups=self.groups,
...
def NFNetF0(num_classes=None,
            ...
            name='NFNet',
            include_top=True):
    model = NFNet(num_classes=num_classes,
                  variant='F0',
                  width=width,
                  se_ratio=se_ratio,
                  alpha=alpha,
                  ...
                  fc_init=fc_init,
                  final_conv_mult=final_conv_mult,
                  final_conv_ch=final_conv_ch,
                  use_two_convs=use_two_convs,
                  name=name,
                  include_top=include_top)
    sample_data = np.random.rand(1, 224, 224, 3)
    pred = model(sample_data, training=False)
    return model
