...
from fastestimator.op.numpyop.multivariate import HorizontalFlip, LongestMaxSize, SmallestMaxSize
...
from fastestimator.util import Suppressor, get_num_devices, pad_batch
...
class PadMulti(NumpyOp):
    ...
    def forward(self, data, state):
        ...
        mask = np.pad(mask, pad_width=((0, pad_height - height), (0, pad_width - width), (0, 0)))
        return image, mask
...
class Gt2Target(NumpyOp):
    ...
    def forward(self, data, state):
        masks, bboxes = data
        # downscale mask for memory efficiency
        masks = cv2.resize(masks, (masks.shape[1] // 4, masks.shape[0] // 4))
        if len(masks.shape) == 2:
            masks = np.expand_dims(masks, axis=-1)  # cv2 will remove last dim if there's single object
        masks = np.transpose(masks, [2, 0, 1])  # (H, W, #objects) -> (#objects, H, W)
        # start assigning ground truth to grids
    ...
    def forward_batch(self, data, state):
        masks_batch, bboxes_batch = data
        batch_data = [self.forward([masks, bboxes], state) for masks, bboxes in zip(masks_batch, bboxes_batch)]
        # pad dimension according to shape of largest element
        batch_data = [{"gt_match": batch[0], "masks": batch[1], "classes": batch[2]} for batch in batch_data]
        pad_batch(batch_data, pad_value=0)
        # re-stack values into batch again
        gt_match_batch = np.stack([batch['gt_match'] for batch in batch_data])
        masks_batch = np.stack([batch['masks'] for batch in batch_data])
        classes_batch = np.stack([batch['classes'] for batch in batch_data])
        return gt_match_batch, masks_batch, classes_batch
...
class ClipMax(NumpyOp):
    def __init__(self, resize_op):
        super().__init__(inputs=resize_op.inputs, outputs=resize_op.outputs, mode=resize_op.mode)
        self.max_size = resize_op.func[0].max_size
        self.resize_op = resize_op

    def forward(self, data, state):
        max_size = max(data[0].shape[0], data[0].shape[1])
        if max_size > self.max_size:
            data = self.resize_op.forward(data, state)
        return data
...
class COCOMaskmAP(Trace):
    def __init__(self, data_dir, im_scale, max_size, inputs=None, outputs="mAP", mode=None):
        super().__init__(inputs=inputs, outputs=outputs, mode=mode)
        with Suppressor():
    ...
    def on_batch_end(self, data):
        ...
        for seg_pred, cate_score, cate_label, image_id, imsize in zip(seg_preds, cate_scores, cate_labels, image_ids, imsizes):
            ...
            seg_pred = cv2.resize(seg_pred, (4 * mask_w, 4 * mask_h))
            if num_obj == 1:
                seg_pred = seg_pred[..., np.newaxis]  # when there's only single object, resize will remove the channel
            # calculate scale ratio from original image size to new size, to figure out how much padding applied
            imsize_min, imsize_max = np.min(imsize), np.max(imsize)
            scale_ratio = self.im_scale / imsize_min
            if np.round(scale_ratio * imsize_max) > self.max_size:
                scale_ratio = self.max_size / imsize_max
            imsize_resized = imsize * scale_ratio  # this is the shaper before the 32-multiply padding
            seg_pred = seg_pred[:round(imsize_resized[0]), :round(imsize_resized[1]), :]
            # now reshape to original shape
            seg_pred = cv2.resize(seg_pred, (imsize[1], imsize[0]))
            if num_obj == 1:
                seg_pred = seg_pred[..., np.newaxis]  # when there's only single object, resize will remove the channel
...
def get_estimator(data_dir=None,
    ...
    pipeline = fe.Pipeline(
        ...
        ops=[
            ...
            SmallestMaxSize(im_scale, image_in="image", mask_in="mask", bbox_in="bbox", bbox_params="coco"),
            ClipMax(LongestMaxSize(max_size, image_in="image", mask_in="mask", bbox_in="bbox", bbox_params="coco")),
    ...
    model = fe.build(
        model_fn=lambda: solov2(input_shape=(None, None, 3)),
