...
def conv(input_data, filters_shape, trainable, name, downsample=False, activate=True, bn=True, act_fun='leaky_relu'):
    with tf.compat.v1.variable_scope(name):
        ...
        weight = tf.compat.v1.get_variable(name='weight',
                                           dtype=tf.float32,
                                           trainable=True,
                                           shape=filters_shape,
...
def upsample(input_data, name, method='deconv'):
    ...
    if method == 'deconv':
        ...
        output = tf.layers.conv2d_transpose(input_data,
                                            num_filter,
                                            kernel_size=2,
                                            padding='same',
                                            strides=(2, 2),
...
def cspstage(input_data, trainable, filters, loop, layer_nums, route_nums, res_nums):
    ...
    input_data = conv(input_data, (1, 1, c, int(c / 2)),
                      trainable=trainable,
                      name='conv%d' % (layer_nums + 1),
                      act_fun='mish')

    for i in range(loop):
        input_data = res_block(input_data,
                               int(c / 2),
                               int(c / 2),
                               int(c / 2),
                               trainable=trainable,
    ...
    input_data = conv(input_data, (1, 1, int(c / 2), int(c / 2)),
                      trainable=trainable,
                      name='conv%d' % out_layer,
                      act_fun='mish')
...
def cspdarknet53(input_data, trainable, init_width_size, init_depth_size):
    ...
    input_data = conv(input_data, (1, 1, init_width_size, 2 * init_width_size),
                      trainable=trainable,
                      name='conv1',
                      downsample=True,
                      act_fun='mish')
    ...
    input_data = conv(input_data, (3, 3, 2 * init_width_size, 4 * init_width_size),
                      trainable=trainable,
    ...
    input_data = conv(input_data, (3, 3, 4 * init_width_size, 8 * init_width_size),
                      trainable=trainable,
                      name='conv%d' % (layer_num + 1),
                      downsample=True,
                      act_fun='mish')
    ...
    input_data = conv(input_data, (3, 3, 8 * init_width_size, 16 * init_width_size),
                      trainable=trainable,
                      name='conv%d' % (layer_num + 1),
                      downsample=True,
                      act_fun='mish')
    ...
    input_data = conv(input_data, (1, 1, 64 * init_width_size, 16 * init_width_size),
                      trainable=trainable,
...
class YOLOV5(object):
    def __init__(self, input_data, trainable):
        ...
        self.anchors = np.array([[10, 13, 16, 30, 33, 23], [30, 61, 62, 45, 59, 119], [116, 90, 156, 198, 373,
                                                                                       326]]).reshape(3, 3, 2)
        self.anchor_per_scale = 3
        self.iou_loss_thresh = 0.5
        self.upsample_method = 'resize'
        self.width_scale = 0.50
        self.depth_scale = 0.33
    ...
    def __build_network(self, input_data):
        ...
        y19_1 = conv(y19, (1, 1, 16 * init_width_size, 8 * init_width_size),
                     self.trainable,
                     name='conv%d' % (layer_num + 1))
        ...
        with tf.compat.v1.variable_scope('route_0'):
        ...
        y38 = conv(y38, (1, 1, 16 * init_width_size, 8 * init_width_size),
                   self.trainable,
                   name='conv%d' % (layer_num + 2))
        ...
        # 512x76x76 -> 256x76x76
        y38_1 = conv(y38, (1, 1, 8 * init_width_size, 4 * init_width_size),
                     self.trainable,
                     name='conv%d' % (layer_num + 1))
        ...
        y76 = conv(y76, (1, 1, 8 * init_width_size, 4 * init_width_size),
                   self.trainable,
                   name='conv%d' % (layer_num + 2))
        ...
        y76_downsample = conv(y76, (1, 1, 4 * init_width_size, 4 * init_width_size),
                              trainable=self.trainable,
                              name='downsample0',
                              downsample=True)
        y76_output = conv(y76_downsample, (1, 1, 4 * init_width_size, 3 * (self.num_class + 5)),
                          trainable=self.trainable,
                          name='conv_sbbox',
                          activate=False,
                          bn=False)
        ...
        # 38x38 head/neck
        # 256x152x152 -> 256x76x76
        y38_1 = conv(y76, (3, 3, 4 * init_width_size, 4 * init_width_size),
                     self.trainable,
                     name='conv%d' % (layer_num + 1),
                     downsample=True)
        ...
        # 768x76x76 -> 512x76x76
        y38 = conv(y38, (1, 1, 12 * init_width_size, 8 * init_width_size),
                   self.trainable,
                   name='conv%d' % (layer_num + 2))
        ...
        # 512x76x76 -> 512x38x38
        y38_downsample = conv(y38, (1, 1, 8 * init_width_size, 8 * init_width_size),
                              trainable=self.trainable,
                              name='downsample1',
                              downsample=True)
        y38_output = conv(y38_downsample, (1, 1, 8 * init_width_size, 3 * (self.num_class + 5)),
                          trainable=self.trainable,
                          name='conv_mbbox',
                          activate=False,
                          bn=False)
        ...
        # 19x19 head/neck
        # 512x76x76 -> 512x38x38
        y19_1 = conv(y38, (3, 3, 8 * init_width_size, 8 * init_width_size),
                     self.trainable,
                     name='conv%d' % (layer_num + 1),
                     downsample=True)
        with tf.compat.v1.variable_scope('route_3'):
        ...
        y19 = conv(y19, (1, 1, 24 * init_width_size, 16 * init_width_size),
                   self.trainable,
                   name='conv%d' % (layer_num + 2))
        ...
        y19_downsample = conv(y19, (1, 1, 16 * init_width_size, 16 * init_width_size),
                              trainable=self.trainable,
                              name='downsample2',
                              downsample=True)
        y19_output = conv(y19_downsample, (1, 1, 16 * init_width_size, 3 * (self.num_class + 5)),
                          trainable=self.trainable,
                          name='conv_lbbox',
                          activate=False,
                          bn=False)
    ...
    def decode(self, conv_ouput, anchors, strides):
        ...
        conv_ouput = tf.reshape(conv_ouput,
    ...
    def bbox_ciou(self, boxes1, boxes2):
        ...
        v = 4 / (np.pi * np.pi) * (tf.square(
            tf.math.atan2(boxes1_1[..., 2], boxes1_1[..., 3]) - tf.math.atan2(boxes2_1[..., 2], boxes2_1[..., 3])))
        alp = v / (1.0 - iou + v)
        ciou = diou - alp * v
        return ciou
    ...
    def compute_loss(self,
        ...
        with tf.name_scope('smaller_box_loss'):
            loss_sbbox = self.yolov5_loss(self.conv_sbbox,
                                          self.pred_sbbox,
                                          label_sbbox,
                                          true_sbbox,
        ...
        with tf.name_scope('medium_box_loss'):
            loss_mbbox = self.yolov5_loss(self.conv_mbbox,
                                          self.pred_mbbox,
                                          label_mbbox,
                                          true_mbbox,
        ...
        with tf.name_scope('lager_box_loss'):
            loss_lbbox = self.yolov5_loss(self.conv_lbbox,
                                          self.pred_lbbox,
                                          label_lbbox,
                                          true_lbbox,
...
if __name__ == "__main__":
    inputs = tf.convert_to_tensor(np.random.rand(32, 640, 640, 3), dtype=tf.double)
    model = YOLOV5(inputs, trainable=True)
    pdb.set_trace()
