...
if __name__ == "__main__":
    ...
    converter = tf.lite.TFLiteConverter.from_keras_model(model_yolov5)
    yolov5_tflite_model = converter.convert()
    with open("yolov5.tflite", 'wb') as f:
        f.write(yolov5_tflite_model)

    converter = tf.lite.TFLiteConverter.from_keras_model(model_retinanet)
    retinanet_tflite_model = converter.convert()
    with open("retinanet.tflite", 'wb') as f:
        f.write(retinanet_tflite_model)
